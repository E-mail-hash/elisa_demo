...
def loadDefaultParams(CmatFileName = "", Cmat = [], DmatFileName = "", Dmat = [], signalV = 7.5, singleNode = 0, lookupTableFileName = None, seed=None):
    '''
    ...
    if lookupTableFileName is None:
        lookupTableFileName = 'models/aln/aln-precalc/quantities_cascade.h5' # aLN transfer functions
    ...
    params.duration     = 20000 # Simulation duration (ms)
    ...
    params.density = 1.0 # SC matrix density (< 1 = cut off weakest links)
    ...
    if singleNode:
        ...
        if CmatFileName != "": 
            try:
                Cmat_initFile       = h5py.File(CmatFileName,'r')
            except:
                Cmat_initFile       = scipy.io.loadmat(CmatFileName)
        
            Cmat           = np.array(Cmat_initFile[Cmat_initFile.keys()[0]])
            try:
                Cmat_initFile.close()
            except:
                pass
            N = len(Cmat)
        else: # take Cmat from function argument 
            params.Cmat = Cmat
            N = len(params.Cmat)
        np.fill_diagonal(Cmat,0) 
        params.Cmat         = Cmat  / np.max(Cmat)    # relative coupling strengths (values between 0 and 1), # Cmat(i,j) connnection from jth to ith            
        
        if DmatFileName != "":    
            # Load delay matrix
            try:
                lengthMatFile       = h5py.File(DmatFileName,'r')
            except:
                lengthMatFile       = scipy.io.loadmat(DmatFileName)
        
            #lengthMatFile           = np.array(Cmat_initFile[Cmat_initFile.keys()[0]])
            params.lengthMat    = np.array(lengthMatFile[lengthMatFile.keys()[0]])
            try:
                lengthMatFile.close()
            except:
                pass
        else:                                                           # Cmat(i,j) connnection from jth to ith
    ...
    params.signalV      = 7.5
    params.c_gl         = 0.4    # Postsynaptic potential amplitude for global connections between areas(unitless)
    params.Ke_gl        = 250.   # number of incoming E connections (to E population) from each area
    
    params.surfaceSC    = 0.0    # mix in surfaceSC to structuralSC (experimental)
    params.interhem_strength    = 0.0    # artificial interhemispheric connections (experimental)
    ...
    params.sigma_ou     = 0.00  # mV/ms/sqrt(ms)
